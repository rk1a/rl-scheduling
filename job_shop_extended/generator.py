...
from job_shop_extended.instance_parser import parse_job_shop_instance
...
class ToyGenerator(Generator):
    ...
    def __call__(self, key: chex.PRNGKey) -> State:
        ...
        machines_slowdown = jnp.full(self.num_machines, False, dtype=jnp.bool)
        ...
        state = State(
            ...
            machines_slowdown=machines_slowdown,
            action_mask=None,
...
class InstanceGenerator(Generator):
    """Returns the given benchmark instance."""

    def __init__(self, instance_path: pathlib.Path | str):
        num_jobs, num_machines, processing_times, machine_sequence = (
            parse_job_shop_instance(instance_path)
        )
        ops_durations = jnp.array(
            processing_times,
            jnp.int32,
        )
        duration_zero_mask = ops_durations == 0
        self.ops_durations = jnp.where(duration_zero_mask, -1, ops_durations)
        ops_machine_ids = jnp.array(
            machine_sequence,
            jnp.int32,
        )
        self.ops_machine_ids = jnp.where(duration_zero_mask, -1, ops_machine_ids)

        super().__init__(
            num_jobs, num_machines, num_machines, int(self.ops_durations.max())
        )
    ...
    def __call__(self, key: PRNGKey) -> State:
        ...
        machines_slowdown = jnp.full(self.num_machines, False, dtype=jnp.bool)
        scheduled_times = jnp.full((self.num_jobs, self.max_num_ops), -1, jnp.int32)
        ops_mask = self.ops_machine_ids != -1
        step_count = jnp.array(0, jnp.int32)

        state = State(
            ops_machine_ids=self.ops_machine_ids,
            ops_durations=self.ops_durations,
            ops_mask=ops_mask,
            machines_job_ids=machines_job_ids,
            machines_remaining_times=machines_remaining_times,
            machines_slowdown=machines_slowdown,
...
class RandomDurationInstanceGenerator(InstanceGenerator):
    """Modifies operation durations randomly up to the given fraction of the original duration."""

    def __init__(
        self, instance_path: pathlib.Path | str, max_duration_change: float = 0.5
    ):
        super().__init__(instance_path)
        self.max_duration_change = max_duration_change

    def __call__(self, key: jax.random.PRNGKey) -> State:
        key, duration_noise_key = jax.random.split(key, 2)
        state = super().__call__(key)

        ops_durations = state.ops_durations

        min_duration_change = -jnp.round(ops_durations * self.max_duration_change)
        max_duration_change = jnp.round(ops_durations * self.max_duration_change)
        duration_noise = jax.random.randint(
            duration_noise_key,
            shape=ops_durations.shape,
            minval=min_duration_change,
            maxval=max_duration_change,
        )
        state.ops_durations = ops_durations + duration_noise
        return state
