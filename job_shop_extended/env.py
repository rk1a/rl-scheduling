...
from job_shop_extended.generator import Generator, RandomGenerator
from job_shop_extended.types import Observation, State
...
class JobShop(Environment[State, specs.MultiDiscreteArray, Observation]):
    ...
    def __init__(
        ...
        slowdown_probability: float = 0.,
        slowdown_factor: float = 0.
    ):
        ...
        Args:
            ...
            slowdown_probability: Probability per step of a machine slowing down.
            slowdown_factor: Factor by which the processing durations increase on slowdown.
        """
        ...
        self.slowdown_percentage = slowdown_factor
        super().__init__()
    ...
    def step(
        ...
        updated_machines_slowdown, updated_ops_durations, updated_key = (
            self._sample_machines_slowdown(state)
        )
        ...
        next_state = State(
            ...
            ops_durations=updated_ops_durations,
            ...
            machines_slowdown=updated_machines_slowdown,
    ...
    def _sample_machines_slowdown(
        self, state: State
    ) -> tuple[chex.Array, chex.Array, chex.PRNGKey]:
        key, slowdown_key = jax.random.split(state.key)
        current_machines_slowdown = jnp.where(
            state.machines_slowdown,
            False,
            jax.random.bernoulli(
                slowdown_key,
                self.slowdown_probability,
                shape=state.machines_slowdown.shape,
            ),
        )

        def slowed_down_ops_for_machine(machine_id: chex.Array) -> chex.Array:
            return current_machines_slowdown[machine_id] & (state.ops_machine_ids == machine_id)

        slowed_down_ops_masks = jax.vmap(slowed_down_ops_for_machine)(jnp.arange(self.num_machines))
        # sum up the masks along the vmap axis
        slowed_down_ops_mask = slowed_down_ops_masks.sum(axis=0)
        # only slow down operations that are yet to be scheduled
        # somewhat unrealistically currently processed operations are not slowed down
        slowed_down_ops_mask = slowed_down_ops_mask & state.ops_mask
        updated_ops_durations = jnp.where(
            slowed_down_ops_mask,
            jnp.round(state.ops_durations * (1 + self.slowdown_percentage)).astype(dtype=jnp.int32),
            state.ops_durations
        )

        updated_machines_slowdown = state.machines_slowdown | current_machines_slowdown
        return updated_machines_slowdown, updated_ops_durations, key
